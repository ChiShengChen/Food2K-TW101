...
model.head.fc = torch.nn.Linear(in_features=512, out_features=NUM_FINETUNE_CLASSES, bias=True)
# print(model)
...
optimizer = torch.optim.AdamW(model.parameters(), lr=0.00001, eps=1e-5)
scheduler = lr_scheduler.StepLR(optimizer, step_size=2, gamma=0.9)
train(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100)
